...
**Для изменений контента (через Nuxt Studio или git):**

1. Push в `main` ветку
2. GitHub Actions подключается к серверу по SSH
3. Выполняется `git pull` и `pm2 restart`
4. Контент обновляется мгновенно - **сборка не нужна**

**Для изменений кода (компоненты, конфиг):**

Требуется полная пересборка на сервере вручную:
...
- `VDS_PATH` - путь на сервере (`/var/www/track-licence`)
